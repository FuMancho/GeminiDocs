...
> [!NOTE]
> Gemini 3.1 Pro Preview is rolling out. To determine whether you have access to Gemini 3.1, use the `/model` command and select **Manual**. If you have access, you will see `gemini-3.1-pro-preview`.
...
Learn more about [models](https://geminicli.com/docs/cli/model/) and [model routing](https://geminicli.com/docs/cli/model-routing/).
...
> [!NOTE]
> The **Keep trying** option uses exponential backoff, in which Gemini CLI waits longer between each retry, when the system is busy. If the retry doesn't happen immediately, please wait a few minutes for the request to process.
...
1. Navigate to the Google Cloud Project you're using with Gemini CLI for Code Assist.
2. Go to **Admin for Gemini** > **Settings**.
...
3. Set **Preview Features** to `true`.
4. Restart Gemini CLI and you should have access to Gemini 3.

## Need help?

If you need help, we recommend searching for an existing [GitHub issue](https://github.com/google-gemini/gemini-cli/issues). If you cannot find a GitHub issue that matches your concern, you can [create a new issue](https://github.com/google-gemini/gemini-cli/issues/new).

For comments and feedback, consider opening a [GitHub discussion](https://github.com/google-gemini/gemini-cli/discussions).
